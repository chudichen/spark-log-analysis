...
public class SessionAggStatAccumulator implements AccumulatorParam<String> {
    ...
    /**
     * zero方法，其实主要用于数据的初始化，那么，我们这里，就返回一个值，
     * 就是初始化中，所有范围区间的数量，都是0，各个范围区间的统计数量的
     * 拼接，还是采用一如既往的key=value|key=value的连接串的格式
     *
     * @param value 初始化
     * @return 初始化值
     */
    @Override
    public String zero(String value) {
     ...
     * 可以理解为是一样的
     *
     * 这两个方法，其实主要就是实现，v1可能就是我们初始化的那个连接串v2,
     * 就是我们在遍历session的时候，判断出某个session对应的区间，然后会用Constants.TIME_PERIOD_1s_3s
     * 所以，我们要做的事情就是在v1中，找到v2对应的value，累加1，然后再更新会连接串里面去
     *
     * @param v1 连接串
     * @param v2 目标
     * @return 拼接后的字符串
